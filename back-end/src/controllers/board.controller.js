...
export const getBoards = async (req, res) => {
  try {
    ...
    if (boards.length <= 0) return res.status(404).json(["boards not found"]); // Imprime un error si no se encuentran tableros.
    res.json(boards);
...
export const getBoardById = async (req, res) => {
  try {
    ...
    if (!board) { // Imprime un error si no se encuentra el tablero.
...
export const deleteBoard = async (req, res) => {
  try {
    ...
    if (deleteBoard_user <= 0) // Imprime un error si el tablero no coincide con el usuario.
...
export const inviteBoard = async (req, res) => {
  try {
    ...
    if (userExist) // Imprime un error si el usuario ya se encuentra en el tablero.
    ...
    if (!invitedBoard) // Imprime un error si no se encuentra el tablero.
